...
a graph. The functions are just-in-time compiled using Numba for faster performance.
'''
...
def eisner(scores: np.ndarray) -> np.ndarray:
    ...
    Args:
        scores: An n x n NumPy array containing scores for every possible arc.
...
def _backtrack(backpointers: np.ndarray, s: int, t: int,
    ...
    Args:
        ...
        head: An int containing the index for head direction.
            RIGHT = 0, LEFT = 1.
        arcs: A size n NumPy array indexed by the dependent ID.
    """
